...
if (G<=Gth){
    ...
    set yrange [2*GPVAL_Y_MIN:2*GPVAL_Y_MAX];
    }
...
set style line 8 dt (18,6) lc rgb "purple" lw 4;
set style line 9 dt (18,6) lc rgb "dark-green" lw 4;
# set size 2,1.;
...
if (G!=0) { 
    ...
    set label 33 at first 605, graph 0.95 "{/=14 PUMP ON}";
    set object 66 rect from graph 0, graph 0 to graph 1, graph 1  fc rgb "light-blue" fillstyle transparent solid;
    
set xlabel "Time (ps)"
set key bottom left
plot "../data/output/anltime.dat" u 1:2 w l ls 8 t "analytical {/Symbol a}'",\
     "../data/output/anltime.dat" u 1:3 w l ls 9 t "analytical {/Symbol a}''",\
     "../data/output/numtime.dat" u 1:2 w l ls 6 t "numerical {/Symbol a}'",\
